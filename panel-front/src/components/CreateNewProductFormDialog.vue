<template>
     <v-dialog transition="dialog-top-transition" max-width="500" v-model="showDialog" persistent>
        <v-card :loading="loading">
            <v-card-title>Criar novo Produto</v-card-title>
            <v-card-text>
            ...
            <v-card-actions style="padding-bottom: 20px;">
                <v-spacer></v-spacer>
                <v-btn color="primary darken-4" dark @click="createNewProduct()">Registrar</v-btn>
    ...
    methods: {
        ...
        async createNewProduct() {
            try {
                ...
                this.loading = true;
                const response = await ProductsService.create(this.product);
                if(response.status == 201) {
                    this.close()
                    window.alert("Cadastro de producto realizado com sucesso!");
                    return;
                } 
